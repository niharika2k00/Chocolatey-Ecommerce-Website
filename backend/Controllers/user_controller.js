...
const userAuth = asyncHandler(async (req, res) => {
  ...
  if (user && (await user.matchPassword(password))) {
    // receive the returned value from ModelUser
    res.json({
      // req.json returns the Object
      _id: user._id,
      name: user.name,
      email: user.email,
...
const userRegister = asyncHandler(async (req, res) => {
  ...
  if (userExists) {
    ...
    if (new_person) {
      res.status(201).json({
        // Successfully created & led to the creation of a resource
        _id: new_person._id,
...
const updateUserProfile = asyncHandler(async (req, res) => {
  ...
  if (user) {
    // user --> Original user thats stored previously
